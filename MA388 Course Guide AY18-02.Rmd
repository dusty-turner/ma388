...
Essentially, $\hat{S}_{(t)}$ is the conditional probability of death at any given time $t$
...
3. Calculate and draw a Kaplan-Meier estimate for the following data:

|Obstacle|Fail Obstacle|Timed Out (Censored)|
|-------------------|-----------------|-----------|
|Piston Road| 0 | 0 |
|Giant Ring| 2 | 0 |
|Silk Slider| 21 | 0 |
...
managers = managers %>%
  select(playerID, totalseasons, censor)

survobj = Surv(managers$totalseasons,managers$censor)
survobj[1:50]
```

No create the Kaplan-Meier Estimate Survivor Function
...
2. Interpret Hazard Ratio and convey its meaning in terms of real world data.  
...
The hazard function measures the failure ratio over the survival ratio.
