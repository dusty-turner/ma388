...
texas = texas[-1,]
texas %>% as.tibble() %>% write.csv("texas.csv", row.names = FALSE)

texas = read_csv("texas.csv") %>% filter(!is.na(Rk)) %>% write.csv("texas.csv", row.names = FALSE)
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
library(tidyverse)
fielding = read_csv("Fielding.csv") %>% mutate(lgID = as.character(lgID))
